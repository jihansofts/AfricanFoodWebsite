    ...
    <nav className="bg-background shadow-lg sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          {/* Mobile Menu Button */}
          <motion.button
            className="md:hidden p-2 rounded-md text-gray-600 hover:text-primary hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-inset"
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              ...
              <div className="px-2 pt-2 pb-4 space-y-1 bg-background border-t border-gray-200">
                {navItems.map((item) => (
                  <motion.button
                    key={item.id}
                    onClick={() => handleLinkClick(item.id)}
                    className={`block w-full text-left px-3 py-3 rounded-md text-[16px] font-inter transition-all duration-200 ${
                      activeLink === item.id
                        ? "text-primary font-bold bg-blue-50 border-l-4 border-primary"
                        : "text-text font-normal hover:text-primary hover:bg-gray-50"
                    }`}
                    whileHover={{ x: 8 }}
                    whileTap={{ scale: 0.98 }}
